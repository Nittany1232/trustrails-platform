...
export const syncDOLData: HttpFunction = async (req, res) => {
  ...
  // Use 2024 data (latest available as of 2025)
  const year = 2024; // Hardcode to 2024 for now

  try {
    // 1. Validate request (accept from Cloud Scheduler or with proper auth)
    const userAgent = req.headers['user-agent'];
    const isScheduler = userAgent && userAgent.includes('Google-Cloud-Scheduler');
    const authHeader = req.headers['authorization'];

    if (process.env.NODE_ENV === 'production' && !isScheduler && !authHeader) {
      console.log('❌ Unauthorized request - not from scheduler');
      return res.status(403).json({ error: 'Unauthorized' });
    }

    console.log(`✅ Request authorized (scheduler: ${isScheduler})`);

    // 2. Check if sync already ran this month (skip for manual testing)
    ...
    // Allow force resync with query parameter
    const forceResync = req.query.force === 'true';
    ...
    if (!forceResync) {
      ...
      console.log('⚠️ Force resync requested');
    }
